  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Juba Logistics Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Juba Logistics Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest external-beneficiary-registration-noncompliant-is-held-and-unoverridable
  (testing "declared :external-beneficiary-procurement? true but not all three registration facts hold -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:external-beneficiary-registration-noncompliant} (-> (store/ledger db) last :basis)))
...
(deftest tin-record-unverified-is-held-and-unoverridable
  (testing "unverified TIN record when required -> HARD hold"
    (let [[db actor] (fresh)
